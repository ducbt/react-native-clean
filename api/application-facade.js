...
function signIn(req, res) {
  if (req.body.password === 'password1') {
    success(res, 'got activity stages')({success:true, message: 'user signIn successful', token: '123'});
  }else {
    success(res, 'got activity stages')({success: false, message: 'user signIn failed'});
  }
}
